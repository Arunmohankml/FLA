...
export function CourseClient({ course, otherCourses }: CourseClientProps) {
  ...
  return (
    <>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-8 lg:p-12">
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                <div className="rounded-xl bg-white/15 px-4 py-2.5 backdrop-blur-sm">
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-white/50">Levels</p>
                ...
                <div className="rounded-xl bg-white/15 px-4 py-2.5 backdrop-blur-sm">
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-white/50">Mode</p>
                  <p className="text-sm font-bold text-white">Hybrid</p>
                </div>
                <Link
                  href="/register"
                  className="ml-auto hidden items-center rounded-full bg-[#e8734a] px-6 text-sm font-semibold text-white transition-all duration-300 hover:bg-[#d65a30] sm:inline-flex"
                >
                  Register Now →
                </Link>
              </div>
        ...
        <div className="page-shell">
          ...
          >
            <motion.div variants={fadeUp}>
              <span className="mb-3 inline-flex items-center gap-2 rounded-full border border-black/8 bg-[#faf5f0] px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.2em] text-black/50">
                Why This Language
              </span>
              ...
              <ul className="mt-8 space-y-4">
                {course.whyLearn.map((item, i) => (
                  <motion.li key={i} variants={fadeUp} className="flex items-start gap-3">
                    <span className="mt-1 flex size-6 shrink-0 items-center justify-center rounded-full bg-black/[0.04] text-xs font-bold text-black/40">
                      {i + 1}
                    </span>
      ...
      {/* ── Course Levels ── */}
      <section className="bg-[#faf5f0] page-section py-16 lg:py-24">
        <div className="page-shell">
          <span className="mb-3 inline-flex items-center gap-2 rounded-full border border-black/8 bg-white px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.2em] text-black/50">
            Curriculum
          </span>
          <h2 className="font-heading text-3xl font-medium leading-[1.33] tracking-[-0.02em] text-foreground sm:text-4xl">
            Course levels
            ...
            {course.levelDetails.map((level, i) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="rounded-full bg-[#e8734a] px-3 py-1 text-[11px] font-semibold text-white">
                    {level.title}
                  </span>
                </div>
                ...
                <div className="mt-4 flex flex-wrap gap-1.5">
                  {level.skills.map((skill, j) => (
                    <span
                      key={j}
                      className="rounded-full bg-[#faf5f0] px-2.5 py-1 text-[11px] font-medium text-muted-foreground"
                    >
                      {skill}
                    </span>
                  ))}
        ...
        <div className="page-shell">
          ...
          >
            {course.teachingMethod.map((method, i) => (
              ...
              >
                <span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-black/[0.04] text-xs font-bold text-black/40">
                  {i + 1}
                </span>
                <span className="text-sm leading-relaxed text-muted-foreground">{method}</span>
              </motion.div>
            ))}
          </motion.div>
        </div>
            ...
            >
              {course.exams.map((exam, i) => (
                ...
                >
                  ...
                  <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{exam.description}</p>
                </motion.div>
              ))}
            </motion.div>
          </div>
          ...
          >
            {course.careers.map((career, i) => (
              <motion.div
                ...
                className="group rounded-[24px] border border-black/5 bg-white p-6 transition-all duration-300 hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)]"
              >
                <div className="flex size-10 items-center justify-center rounded-2xl bg-black/[0.04] text-sm font-bold text-black/40">
                  {i + 1}
                </div>
            ...
            {course.faq.map((faq, i) => (
              ...
              >
                <summary className="flex cursor-pointer items-center justify-between px-6 py-5 text-base font-semibold">
                  {faq.question}
                  <span className="ml-4 size-6 shrink-0 rounded-full bg-[#faf5f0] flex items-center justify-center text-sm transition-transform duration-200 group-open:rotate-45">
                    +
                  </span>
                </summary>
      ...
      {/* ── CTA ── */}
      <section className="bg-[#1a1a1a] page-section py-16">
